...
struct iio_chan_spec {
	...
	/*用户可以自定义，但是一般会设置为此通道对应的芯片数据寄存器地址*/
	unsigned long address;

	int scan_index;/*当使用触发缓冲区的时候， scan_index 是扫描索引*/
	struct {
		char sign;	/*‘u’表示数据为无符号类型，‘s’为有符号类型*/
		u8	realbits;	/*有效位数，如10位ADC，就是 = 10*/

		/*存储位数，有效位数+填充位。
		比如传感器 ADC 是 12 位的，存储的话要用到 2 个字节，填充 4 位
		也就是 16 位， 存储位数 = 16 */
		u8	storagebits;

		/*右移位数，也就是存储位数和有效位数不一致的时候，需要右移的位数，
		这个参数不总是需要，一切以实际芯片的数据手册位数*/
		u8	shift;

		u8	repeat;	/*实际或存储位的重复数量*/

		/*数据的大小端模式，可设置为 IIO_CPU、 IIO_BE(大端)或 IIO_LE(小端)。*/
	...
	/*标记导出的信息由相同类型的通道共享。
	也就是 iio_chan_spec.type 成员变量相同的通道*/
	...
	/*表设计某些信息所有的通道共享，无论这些通道的类型、方向如何，全部共享。*/
	...
	unsigned		indexed:1;	/*indexed 为 1 的时候， channel 为通道索引*/
...
/* Device operating modes 设备操作模式 */
#define INDIO_DIRECT_MODE			0x01	/*提供 sysfs 接口*/
#define INDIO_BUFFER_TRIGGERED		0x02	/*支持缓冲触发*/
#define INDIO_BUFFER_SOFTWARE		0x04	/*支持软件缓冲区*/
#define INDIO_BUFFER_HARDWARE		0x08	/*支持硬件缓冲区*/
#define INDIO_EVENT_TRIGGERED		0x10	/*支持事件触发*/
#define INDIO_HARDWARE_TRIGGERED	0x20	/*支持硬件触发*/
...
struct iio_info {
	...
	/*最终 读 设备内部数据的操作函数，需要程序编写人员去实现的
	indio_dev:	需要读写的 IIO 设备。
	chan:	需要读取的通道
	val, val2, 应用程序从内核空间读取到数据，一般就是传感器指定通道值，
	val 和 val2 共同组成具体值， val 是整数部分， val2 是小数部分(扩大 N 倍后的整数值)
	比如现在有个值为 1.00236, val=1, vla2 理论上 = 0.00236，
	但是我们需要对 0.00236 扩大 N 倍 = 2360, 因此 val=1, val2=2360;
	扩大的倍数我们不能随便设置，而是要使用 Linux 内核里面定义的数据扩大倍数
	include/linux/iio/types.h  	
	mask:	掩码，用于指定我们读取的是什么数据 	
	Linux 内核使用 IIO_CHAN_INFO_RAW 和 IIO_CHAN_INFO_SCALE 这两个宏来表示
	原始值以及分辨率，这两个宏就是掩码。
	至于每个通道可以采用哪几种掩码，这个在我们初始化通道的时候需要驱动编写人员设置好。
	掩码有很多种  */
 ...
 */
/*回调函数，在使能或禁用缓冲区的时候会调用这些函数。如果未指定的话就默认使用 */
...
struct iio_dev {
	...
	int				modes;	/*设备操作模式, 如 INDIO_DIRECT_MODE */
	...
	int					scan_bytes;		/*捕获到(缓冲区)的字节数*/
	struct mutex		mlock;

	/* 扫描位掩码，使用触发缓冲区的时候可以通过设置掩码来确定使能哪些通道，
	使能以后的通道会将捕获到的数据发送到 IIO 缓冲区。*/
	const unsigned long *available_scan_masks;

	unsigned			masklength;

	/* 为缓冲区已经开启的通道掩码。只有这些使能了的通道数据才能被发送到缓冲区。*/
	const unsigned long *active_scan_mask;

	/*扫描时间戳，如果使能以后会将捕获时间戳放到缓冲区里面*/
	bool				scan_timestamp;

	unsigned			scan_index_timestamp;
	struct iio_trigger		*trig;	/*为 IIO 设备当前触发器，当使用缓冲模式的时候*/
	bool				trig_readonly;
	struct iio_poll_func		*pollfunc;	/*在接收到的触发器上运行的函数*/
	struct iio_poll_func		*pollfunc_event;

	struct iio_chan_spec const	*channels;	/*为 IIO 设备通道*/
	int				num_channels;			/*为 IIO 设备的通道数*/

	struct list_head		channel_attr_list;
	struct attribute_group		chan_attr_group;
	const char			*name;				/*为 IIO 设备名字*/
...
一般我们会将自己定义的设备结构体变量作为 iio_dev 的私有数据，
这样可以直接通过 iio_device_alloc 函数同时完成 iio_dev 和 设备结构体变量 的内存申请。 
*/

/** regmap 是 Linux 内核为了减少慢速 I/O 在驱动上的冗余开销，提供了一种通用的接口来操
作硬件寄存器。另外， regmap 在驱动和硬件之间添加了 cache，降低了低速 I/O 的操作次数，提
高了访问效率，缺点是实时性会降低。
 ...
 * @def: Register default value. 默认值
...
比如要将寄存器的 bit1 和 bit2 置 1，
那么 mask = 0X00000011, val = 0Xxxxxxx11。
如果要 清除 寄存器的 bit4 和 bit7，
那么 mask = 0X10010000, val = 0X0xx0xxxx。
...
regval = regval & (!mask) // 清 bit
regval = regval | val     // 设 bit         */
...
struct regmap_config {
	...
	/*可写，寄存器可写的话此回调函数就会被调用，并返回 true。*/
	bool (*writeable_reg)(struct device *dev, unsigned int reg);

	/*可读，寄存器可读的话此回调函数就会被调用，并返回 true。*/
	bool (*readable_reg)(struct device *dev, unsigned int reg);

	/*当寄存器值不能缓存的时候此回调函数就会被调用，并返回 true。*/
	bool (*volatile_reg)(struct device *dev, unsigned int reg);

	/*当寄存器值不能被读出来的时候此回调函数会被调用，比如很多中断状态寄存器读清零，
	...
	unsigned long read_flag_mask;	/*读 标志掩码*/
...
/** 不管是 I2C 还是 SPI 等接口，还是 SOC 内部的寄存器，对于寄存器的操作就两种：读和写。
regmap 提供了最核心的两个读写操作： regmap_read 和 regmap_write 。
这两个函数分别用来读/写寄存器
map: 要操作的 regmap。
reg: 要读的寄存器。
val: 读到的寄存器值		**/

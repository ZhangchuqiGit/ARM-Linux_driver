...
#define DRIVER_NAME "zcq_misc_driver" /* 驱动名字 */
#define MISC_DEVICE_NAME "zcq_misc_device" 	/* MISC 设备名字 /dev/zcq_misc_device */
...
static int leds_init(struct platform_device *dev) // 初始化led的GPIO
{
	int ret = 0;
	int i = 0;
	char led_labels[DEVICE_NUM][20] =
...
static int led_probe(struct platform_device *dev)
{
	...
	led_dev_sptr = kmalloc(DEVICE_NUM * sizeof(struct reg_dev), GFP_KERNEL);
	if(!led_dev_sptr){
		ret = -ENOMEM;
		goto fail;
	}
	memset(led_dev_sptr,0,DEVICE_NUM * sizeof(struct reg_dev));
	...
	for(i=0;i<DEVICE_NUM;i++){
		...
		/* 5、创建设备 */
		led_dev_sptr[i].device = device_create(led_dev_sptr, NULL, 
			MKDEV(numdev_major, numdev_minor + i), NULL, 
			"%s%d", DEVICE_NAME, i);
		if (IS_ERR(led_dev_sptr)) return PTR_ERR(led_dev_sptr);
	}

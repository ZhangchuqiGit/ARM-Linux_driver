...
#include <linux/usb/ch9.h>	/*USB 描述符*/
 ...
 */
/** 
USB 请求块（ USB request block， urb） 
是 USB 设备驱动中用来描述与 USB 设备通信所用的基本载体和核心数据结构。  */
struct urb {
	/** 私有的： 只能由 USB 核心和主机控制器访问的字段 */
	struct kref kref;		/* URB 的引用计数 */
	int unlinked;			/* 取消链接错误代码 */
	...
	unsigned int pipe;				/* 管道/端点 信息 */
	unsigned int stream_id;			/* (in) stream ID */
	int status;						/* (return) non-ISO status */

	unsigned int transfer_flags;	/* (in) URB_SHORT_NOT_OK | ...*/
	/*	这个变量可被设置为不同的位值， 根据这个位， usb 驱动可以设置 urb 传输的状态。
		当 transfer_flags 标志中的 urb_NO_TRANSFER_DMA_MAP 被置位时， 
	USB 核心将使用 transfer_dma 指向的缓冲区而非 transfer_buffer 指向的缓冲区， 
	意味着即将传输 DMA 缓冲区。
		当 transfer_flags 标志中的 urb_NO_SETUP_DMA_MAP 被置位时，
	对于有 DMA 缓冲区的控制 urb 而言， 
	USB 核心将使用 setup_dma 指向的缓冲区而非 setup_packet 指向的缓冲区。*/

	void *transfer_buffer;			/* 发送/接收 数据缓冲区 */
	/*主机控制器为了正确使用这个缓冲区 transfer_buffer，
	必须使用 kmalloc 调用来创建它，	而不是堆栈或者静态数据区*/

	dma_addr_t transfer_dma;		/* 以 DMA 方式 发送/接收 数据缓冲区 */

	struct scatterlist *sg;			/* (in) scatter gather buffer list */
	int num_mapped_sgs;				/* (internal) mapped sg entries */
	int num_sgs;					/* (in) number of entries in the sg list */
	u32 transfer_buffer_length;		/* 发送/接收 数据缓冲区 大小 */
	u32 actual_length;				/* urb 结束后，发送/接收数据的实际长度 */
	unsigned char *setup_packet;	/* 指向控制 urb 的 设置数据包 */
	dma_addr_t setup_dma;			/* 指向控制 urb 的 设置数据包 的 DMA 缓冲区 */
	int start_frame;				/* 同步传输中用于设置或返回初始帧号(ISO) */
	int number_of_packets;			/* 同步传输中等时缓冲区数据(ISO) */
	int interval;					/* urb 被轮询到的时间间隔(对中断和等时(同步)有效)(INT/ISO) */
	int error_count;				/* 同步传输错误数量 */
	void *context;					/* 指向数据点的指针,可在完成处理中被返回，
		可以是在回调函数使用的数据 */
	usb_complete_t complete;		/* 指向完成处理者函数的指针,
		当 urb 被完全传输或发生错误时， 被调用 */
	struct usb_iso_packet_descriptor iso_frame_desc[0];	/* (in) ISO ONLY */
	/* iso_frame_desc 只对同步 urb 有效; 这个结构允许单个 urb 来一次定义多个同步传送。
	它也用来收集每个单独传送的传送状态 */ 
};
...
extern void usb_free_urb(struct urb *urb);	/*销毁*/
#define usb_put_urb usb_free_urb

extern struct urb *usb_get_urb(struct urb *urb);

/*提交给 USB 核心来发送出到 USB 设备
--------------------------------------------------------------------
参数 mem_flags 告诉 USB 核心如何及时分配任何内存缓冲
GFP_ATOMIC 	只要满足以下条件,就应当使用此值。
	1） 调用者处于一个 urb 结束处理例程,中断处理例程,底半部 tasklet 或者一个定时器回调函数；
	2） 调用者持有自旋锁或者读写锁。 注意如果正持有一个信号量,这个值不必要；
	3） current->state 不是 TASK_RUNNING。 除非驱动已自己改变 current 状态,
		否则状态应该一直是 TASK_RUNNING
GFP_NOIO	驱动处于块 I/O 处理过程中， 它还应当用在所有的存储类型的错误处理过程中。
GFP_KERNEL	所有不属于之前提到的其他情况 
--------------------------------------------------------------------
如果函数调用成功,urb 的完成处理者(就是 complete 指定的的函数)被确切地调用一次,
--------------------------------------------------------------------
有三种情况， 一个 urb 可被结束并且使完成函数被调用。
1） urb 被成功发送给设备,并且设备返回正确的确认。 
	对于一个 OUT urb,数据被成功发送, 对于一个 IN urb,
	请求的数据被成功收到。 如果发生这个,urb 中的状态变量被设置为 0。
2） 一些错误连续发生， 当发送或者接受数据从设备中。 被 urb 结构中的 status 变量中的错误值所记录。
3） 这个 urb 被从 USB 核心去链。 
	这发生在要么当驱动告知 USB 核心取消一个已提交的 urb 通过调用 
	usb_unlink_urb 或者 usb_kill_urb， 要么当设备从系统中去除， 
	以及一个 urb 已经被提交给它		     */

...
event设备目录：    /proc/bus/input/devices
event设备信息：    cat /proc/bus/input/devices

I line：这行包含身份信息，显示了 bus type(include/uapi/linux/input.h: input_id : 
	BUS_PCI~BUS_INTEL_ISHTP), vendor， product， version等信息。
N line：这行包含了名字信息。
P line：这行包含了物理设备信息。
S /sys/devices/platform/gpio-keys/input/input1
...
int __must_check input_register_device(struct input_dev *);/*注册 input 设备*/
void input_unregister_device(struct input_dev *); /*注销 input 设备*/
...
/*在每波的每个点上报后需要紧跟一句input_mt_sync(), 
当这波所有点上报后再使用input_sync()进行同步

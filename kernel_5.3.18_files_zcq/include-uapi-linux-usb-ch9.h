...
/** USB 描述符在 include/uapi/linux/usb/ch9.h 中定义,
 * 所有的 USB 描述符， 无论什么平台（Windows， linux， OS 等等） ， 
 * USB 描述符结构体全部完全一样， 不同平台可能变量名称不一样
 
用 linux 命令查看一下 USB 描述符 lsusb :
...
第一列和第二列： 表示主控制器号， 可以看出作者的机器有 4 个 ROOT HUB。
第三列和第四列： USB 的设备编号（USB HUB 也是 USB 设备）,一共有 7 个 USB 设备。
后面几列， 描述具体的设备信息。
作者机器上， 有 Mouse 鼠标， 4个 root hub， 2个 USB Hub。

使用“ lsusb -v ”命令， 可以查看详细的 USB 描述符信息
...
这个请求块用 struct urb 结构描述并且可在“include/linux/usb.h”中找到       */
...
struct usb_device_descriptor {
	__u8  bLength;				/*该描述符 结构体大小（18 字节）*/
	__u8  bDescriptorType;		/*该描述符 类型*/

	__le16 bcdUSB;				/*USB 版本号*/
	__u8  bDeviceClass;			/*设备类代码（由 USB 官方分配）*/
	__u8  bDeviceSubClass;		/*子类代码（由 USB 官方分配）*/
	__u8  bDeviceProtocol;		/*设备协议代码（由 USB 官方分配）*/
	__u8  bMaxPacketSize0;		/*端点 0 的最大包大小（有效大小为8,16,32,64）*/
	__le16 idVendor;			/*生产厂商编号（由 USB 官方分配）*/
	__le16 idProduct;			/*产品编号（制造厂商分配）*/
	__le16 bcdDevice;			/*设备出厂编号*/
	__u8  iManufacturer;		/*设备厂商 字符串索引*/
	__u8  iProduct;				/*产品描述 字符串索引*/
	__u8  iSerialNumber;		/*设备序列号 字符串索引*/
...
struct usb_config_descriptor {
	__u8  bLength;				/*该描述符 结构体大小（9 字节）*/
	__u8  bDescriptorType;		/*该描述符 类型*/

	__le16 wTotalLength;		/*返回的所有数据大小*/
	__u8  bNumInterfaces;		/*接口数量*/
	__u8  bConfigurationValue;	/*Set_Configuration 命令所需要的参数*/
	__u8  iConfiguration;		/*描述该配置 字符串索引*/

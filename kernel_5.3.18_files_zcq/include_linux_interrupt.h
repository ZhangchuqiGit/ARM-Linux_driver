...
#define IRQF_TRIGGER_RISING		0x00000001			/*上升沿 触发中断*/
#define IRQF_TRIGGER_FALLING 	0x00000002		/*下降沿 触发中断*/
#define IRQF_TRIGGER_HIGH		0x00000004			/*高电平 触发中断*/
#define IRQF_TRIGGER_LOW		0x00000008			/*低电平 触发中断*/
...
参数 const char *name：			devname 是中断名称，
							可以通过注册之后可以通过 “ cat /proc/interrupts ” 查看
参数 void *dev： 				dev_id 是设备 
对应的就是中断释放函数 free_irq   */
/** 注册中断处理函数，使能中断 */
...
两者区别:
多了一个 dev 参数; 在设备驱动卸载时，中断会自动释放 ;
如果想单独释放中断，可使用 devm_free_irq()  */
extern int __must_check
...
/* 在 Linux 内核中要想使用某个中断是需要申请的， request_irq 函数用于申请中断，
	注册中断处理函数，request_irq 函数可能会导致睡眠，
	因此在 中断上下文 或者 其他禁止睡眠 的代码段中 不能使用。
	devm_request_irq 函数会激活(使能)中断，所以不需要我们手动去使能中断
可以通过注册之后可以通过 “ cat /proc/interrupts ” 查看
对应的就是中断释放函数 devm_free_irq()   */
/** 注册中断处理函数，使能中断 */
static inline int __must_check

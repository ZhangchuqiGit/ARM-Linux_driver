...
//	#address-cells = <1>;
//	#size-cells = <1>;
//  compatible = "atkalpha-led"; /* 兼容属性 节点名 */
...
static int __init led_init(void) {
  ...
  /* 1、寄存器地址映射 */
#if 0
	/* 1、寄存器地址映射 使用“古老”的 ioremap 函数完成内存映射，将获取到的 regdata 数组中的
寄存器物理地址转换为虚拟地址 */
...
#else
	/* 1、设备树 使用 of_iomap 函数一次性完成读取 reg 属性以及内存映射， of_iomap 函数
是设备树推荐使用的 OF 函数 */
  ...
  /* 5、创建设备 */
  dtsled.device =
      device_create(dtsled.class, NULL, dtsled.devid, NULL, DTSLED_NAME);

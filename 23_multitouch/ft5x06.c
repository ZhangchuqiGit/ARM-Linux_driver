...
/* 多点电容触摸驱动 
内核自带的 FT5426 的驱动文件为 drivers/input/touchscreen/edt-ft5x06.c，此驱动文件不仅仅
能够驱动 FT5426，FT5206、FT5406 这些都可以驱动
*/

/* 电容触摸屏是 IIC 接口的，需要触摸 IC，以正点原子的 ATK7016 为例，其所使用的触
摸屏控制 IC 为 FT5426，因此所谓的电容触摸驱动就是 IIC 设备驱动 
触摸点的信息通过一系列的 ABS_MT 事件(有的资料也叫消息)上报给 linux 内核，只有
ABS_MT 事件是用于多点触摸的，ABS_MT 事件定义在文件 include/uapi/linux/input.h 中，
相关事件如下所示*/

/* 驱动加载成功以后就会生成/dev/input/eventX(X=1,2,3…)，
不同的平台 event 序号不同，也可能是 event3，event4 等，一切以实际情况为准！输入如下
命令查看 event2，也就是多点电容触摸屏上报的原始数据：
hexdump /dev/input/event2
现在用一根手指触摸屏幕的右上角，然后再抬起，理论坐标值为(1023,0)，但是由于触摸误
差的原因，大概率不会是绝对的(1023,0)，应该是在此值附近的一个触摸坐标值
hexdump /dev/input/event1 //查看触摸屏原始数据上报信息
 */

// # 如何使用 Linux 内核自带的多点电容触摸驱动。在使用之前要先将前
// # 面我们自己添加到内核的 ft5x06.c 这 个 文 件 从 内 核 中 去 除 掉 ， 只 需 要 修 改
// # drivers/input/touchscreen/Makefile 这个文件即可，将下面这一行删除掉
// # 内核自带的 FT5426 的驱动文件为 drivers/input/touchscreen/edt-ft5x06.c，此驱动文件不仅仅
// # 能够驱动 FT5426，FT5206、FT5406 这些都可以驱动
...
static int ft5x06_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	set_bit(BTN_TOUCH, ft5x06.input->keybit);
	...
	/*触摸屏想被当成多点屏被处理, 只需要在驱动中给input_dev额外增加以下几个参数*/
...
#if 0
	/*相当于单点屏的ABX_PRESSURE*/
	input_set_abs_params(mcs_data.input, ABS_MT_TOUCH_MAJOR, 0, MAX_SUPPORT_POINTS, 0, 0);
	/*相当于单点屏的ABS_TOOL_WIDTH*/
	input_set_abs_params(mcs_data.input, ABS_MT_WIDTH_MAJOR, 0, MAX_SUPPORT_POINTS, 0, 0); 
#endif
...
fail:
	input_free_device(ft5x06.input);/* 释放 input_dev 资源 */
	return ret;
}
